...
> 目标：从 Spring AI 1.0.0-M6（里程碑版）升级到 1.0.0 GA（稳定版），同步更新 starter 命名规范。

## 升级状态：🔄 进行中（API 迁移待完成）

Spring AI 1.0.0 GA 已在 Maven Central 发布，但包含 breaking API changes 需要逐文件适配。
...
## GA 已知 Breaking Changes

| 原 API (M6) | 新 API (GA) | 影响文件 |
|---|---|---|
...
1. 在独立分支 `upgrade/spring-ai-ga` 上逐文件适配 API 变化
