import { Controller, Get, NotFoundException, Param, Post, Query, Req, Res, StreamableFile } from "@nestjs/common";
...
export class IngestionController {
  ...
  async upload(@TenantId() tenantId: string, @Param("chatId") chatId: string, @Req() request: FastifyRequest) {
  ...
  downloadPdf(
    ...
    const latest = this.ingestionService.latestFileForChat(tenantId, chatId);
  ...
  getJob(@TenantId() tenantId: string, @Param("jobId") jobId: string) {
    const job = this.ingestionService.getJob(tenantId, jobId);
    if (!job) {
  ...
  @Get("ingestion/jobs")
  listJobs(@TenantId() tenantId: string, @Query("chatId") chatId: string, @Query("limit") limit = "20") {
    return this.ingestionService.listByChatId(tenantId, chatId, Number(limit));
  }
